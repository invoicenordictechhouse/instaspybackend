...
def update_existing_google_ads_data(
    ...
    try:
      if update_mode == UpdateMode.ACTIVE:
        store_active_creative_ids_in_staging(
                bigquery_client=bigquery_client, project_id=project_id, dataset_id=dataset_id, clean_table_id=clean_table_id, staging_table_id=staging_table_id
            )
        update_active_ads(bigquery_client=bigquery_client,project_id=project_id,dataset_id=dataset_id,raw_table_id=raw_table_id,staging_table_id=staging_table_id)
      elif update_mode == UpdateMode.ALL:
         update_all_ads(bigquery_client=bigquery_client,project_id=project_id,dataset_id=dataset_id,raw_table_id=raw_table_id,advertiser_ids_table=advertiser_ids_table)
      elif update_mode == UpdateMode.SPECIFIC:
         update_specific_ads(bigquery_client=bigquery_client,project_id=project_id,dataset_id=dataset_id,raw_table_id=raw_table_id,advertiser_ids=advertiser_ids,creative_ids=creative_ids)
