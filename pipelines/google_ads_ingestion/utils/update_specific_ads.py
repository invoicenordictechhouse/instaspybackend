...
def update_specific_ads(
    ...
    conditions = []
    if advertiser_ids:
        advertiser_ids_str = ', '.join([f'"{advertiser_id}"' for advertiser_id in advertiser_ids])
        conditions.append(f"t.advertiser_id IN ({advertiser_ids_str})")

    if creative_ids:
        creative_ids_str = ', '.join([f'"{creative_id}"' for creative_id in creative_ids])
        conditions.append(f"t.creative_id IN ({creative_ids_str})")

    if not conditions:
        raise ValueError("At least one of advertiser_ids or creative_ids must be provided.")
    ...
    try:
        ...
        query_job.result()
        logging.info(f"Specific ads updated successfully based on advertiser_ids: {advertiser_ids} or creative_ids: {creative_ids}")
        print(f"Specific ads updated successfully based on advertiser_ids: {advertiser_ids} or creative_ids: {creative_ids}")
